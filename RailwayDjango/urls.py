...
urlpatterns = [
    ...
    path('contact-us-success/', contact_us_success, name='contact_success'),
    path('search_train/',Search_Train,name="search_train"),
    path('book_detail/(?P<coun>[0-9]+)/(?P<pid>[0-9]+)/(<str:route1>)/(<str:route_type>)/(<str:trip_fare>)/(<str:date_of_return>)',Book_detail,name="book_detail"),
    path('delete_passenger/(?P<pid>[0-9]+)/(?P<bid>[0-9]+)/(<str:route1>)/(<str:route_type>)/(<str:trip_fare>)/(<str:date_of_return>)',Delete_passenger,name="delete_passenger"),
    path('dashboard/',Dashboard,name="dashboard"),
    path('card_detail/(?P<total>[0-9]+)/(?P<coun>[0-9]+)/(<str:route1>)/(?P<pid>[0-9]+)/(<str:date_of_return>)',Card_Detail,name="card_detail"),
    path('log_out/',Logout,name="log_out"),
    path('err/',Errb,name="err"),
    ...
    path('generate-ticket/<int:pid>/', generate_ticket, name='generate_ticket'),
