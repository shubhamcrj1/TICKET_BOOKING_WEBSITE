...
def contact(request):
    if request.method == 'POST':
        ...
        ContactUsForm.objects.create(name=name, email=email, message=message)
        return redirect('contact_success')  # Redirect to a success page

    return render(request, 'contact.html')
def contact_us_success(request):
    return render(request, 'cont_succ.html')
...
def delte_my_booking(request,pid):
    ...
    try:
        ...
        train.seats += 1  # Increase seat count by 1
        train.save()
        passenger.delete()
        error = True
...
def Add_train(request):
    ...
    if request.method == "POST":
        ...
        s = request.POST['seat']
        i = request.FILES['img']
        Add_Train.objects.create(trainname=n,train_no=no,from_city=f,to_city=to,departuretime=de,arrivaltime=a,trevaltime=t,distance=d,seats=s,img=i)
        error=True
    d={"error":error}
...
def generate_ticket(request, pid):
    # Retrieve ticket details based on the provided pid
    ticket = Book_ticket.objects.get(id=pid)
    ...
    p.setFont("Helvetica", 12)  # Set the font and font size

    # Add ticket details
    p.drawString(230, 670, "Ticket Details:")
    p.drawString(180, 625, "Passenger Name: {}".format(ticket.passenger.name))
    p.drawString(180, 600, "Train Name: {}".format(ticket.passenger.train.trainname))
